...
try:
    ...
    # print("PEFT library not found, LoRA training will not be available.")
    get_peft_model = None
    LoraConfig = None
    TaskType = None
...
## GET LLAMA-MODEL
def get_model(model_name_or_path, cache_dir=None, lora_args=None):
    if "t5" in model_name_or_path:
        model = AutoModelForSeq2SeqLM.from_pretrained(model_name_or_path, cache_dir=cache_dir, attn_implementation="eager") # zly
    else:
        model = AutoModelForCausalLM.from_pretrained(model_name_or_path, cache_dir=cache_dir, attn_implementation="eager") # zly	
    if lora_args and get_peft_model:
